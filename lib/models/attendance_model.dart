...
extension AttendanceStatusX on AttendanceStatus {
  String get value {
    switch (this) {
      case AttendanceStatus.checkedIn:       return 'checkedIn';
      case AttendanceStatus.onBreak:         return 'onBreak';
      case AttendanceStatus.checkedOut:      return 'checkedOut';
      case AttendanceStatus.absent:          return 'absent';
      case AttendanceStatus.present:         return 'present';
      case AttendanceStatus.late:            return 'late';
      case AttendanceStatus.halfDay:         return 'halfDay';
      case AttendanceStatus.onLeave:         return 'onLeave';
      case AttendanceStatus.firstHalfLeave:  return 'firstHalfLeave';
      case AttendanceStatus.secondHalfLeave: return 'secondHalfLeave';
    }
  }

  String get label {
    switch (this) {
      case AttendanceStatus.checkedIn:       return 'Checked In';
      ...
      case AttendanceStatus.firstHalfLeave:  return 'First Half Leave';
      case AttendanceStatus.secondHalfLeave: return 'Second Half Leave';
    }
  }

  String get shortLabel {
    switch (this) {
      case AttendanceStatus.checkedIn:       return 'In';
      case AttendanceStatus.onBreak:         return 'Break';
      case AttendanceStatus.checkedOut:      return 'Out';
      case AttendanceStatus.absent:          return 'Absent';
      case AttendanceStatus.present:         return 'Present';
      case AttendanceStatus.late:            return 'Late';
      case AttendanceStatus.halfDay:         return 'Half Day';
      case AttendanceStatus.onLeave:         return 'Leave';
      case AttendanceStatus.firstHalfLeave:  return '½ AM Leave';
      case AttendanceStatus.secondHalfLeave: return '½ PM Leave';
    }
  }
  ...
  int get colorValue {
    switch (this) {
      case AttendanceStatus.checkedIn:       return 0xFF2563EB;
      case AttendanceStatus.onBreak:         return 0xFFF97316;
      case AttendanceStatus.checkedOut:      return 0xFF059669;
      case AttendanceStatus.absent:          return 0xFFDC2626;
      case AttendanceStatus.present:         return 0xFF059669;
      case AttendanceStatus.late:            return 0xFFF59E0B;
      case AttendanceStatus.halfDay:         return 0xFF8B5CF6;
      case AttendanceStatus.onLeave:         return 0xFF0891B2;
      case AttendanceStatus.firstHalfLeave:  return 0xFF7C3AED;
      case AttendanceStatus.secondHalfLeave: return 0xFF0E7490;
    }
  }
  ...
  int get bgColorValue {
    switch (this) {
      case AttendanceStatus.checkedIn:       return 0xFFDBEAFE;
      case AttendanceStatus.onBreak:         return 0xFFFFF7ED;
      case AttendanceStatus.checkedOut:      return 0xFFD1FAE5;
      case AttendanceStatus.absent:          return 0xFFFEE2E2;
      case AttendanceStatus.present:         return 0xFFD1FAE5;
      case AttendanceStatus.late:            return 0xFFFEF3C7;
      case AttendanceStatus.halfDay:         return 0xFFEDE9FE;
      case AttendanceStatus.onLeave:         return 0xFFCFFAFE;
      case AttendanceStatus.firstHalfLeave:  return 0xFFF5F3FF;
      case AttendanceStatus.secondHalfLeave: return 0xFFECFEFF;
    }
  }
  ...
  static AttendanceStatus fromString(String? s) {
    switch (s) {
      ...
      case 'onBreak':         return AttendanceStatus.onBreak;
      ...
      case 'onLeave':         return AttendanceStatus.onLeave;
      case 'firstHalfLeave':  return AttendanceStatus.firstHalfLeave;
      case 'secondHalfLeave': return AttendanceStatus.secondHalfLeave;
...
class BreakEntry {
  ...
  BreakEntry copyWith({DateTime? breakStart, DateTime? breakEnd}) =>
      BreakEntry(
...
class AttendanceModel {
  ...
  Duration get totalWorkDuration {
    ...
    final totalBreak = breaks.fold<int>(
      0,
          (sum, b) => sum +
          (b.breakEnd ?? DateTime.now())
              .difference(b.breakStart)
              .inSeconds,
    );
  ...
  Map<String, dynamic> toMap() => {
    ...
    'checkInTime':  checkInTime  != null
        ? Timestamp.fromDate(checkInTime!)  : null,
    'checkOutTime': checkOutTime != null
        ? Timestamp.fromDate(checkOutTime!) : null,
  ...
  factory AttendanceModel.fromMap(Map<String, dynamic> m) => AttendanceModel(
    ...
    breaks: (m['breaks'] as List<dynamic>? ?? [])
        .map((e) =>
        BreakEntry.fromMap(e as Map<String, dynamic>))
        .toList(),
...
class MonthlyArchive {
  ...
  int get fullDayLeaveDays => days.values
      .where((d) => d.status == AttendanceStatus.onLeave)
      .length;

  /// Each half-day counts as 0.5.
  double get halfDayLeaveCount => days.values
      .where((d) =>
  ...
  double get totalDeductedLeaveDays =>
      fullDayLeaveDays + halfDayLeaveCount;

  double get avgDailyWorkHours {
    final worked =
    days.values.where((d) => d.checkInTime != null).toList();
    if (worked.isEmpty) return 0;
    final total = worked.fold<int>(
        0, (sum, d) => sum + d.totalWorkDuration.inSeconds);
  ...
  double get avgProductivity {
    ...
    days.values.where((d) => d.checkInTime != null).toList();
    if (worked.isEmpty) return 0;
    return worked.fold<double>(
        0, (sum, d) => sum + d.productivityPercent) /
        worked.length;
  }
  ...
  factory MonthlyArchive.fromMap(Map<String, dynamic> m) {
    ...
    return MonthlyArchive(
      ...
      days: daysRaw.map(
            (k, v) => MapEntry(
            k,
            AttendanceModel.fromMap(v as Map<String, dynamic>)),
      ),
    );
  }
  ...
  MonthlyArchive withDay(String dayKey, AttendanceModel record) {
    ...
    return MonthlyArchive(
        userId: userId, year: year, month: month, days: updated);
  }
}
