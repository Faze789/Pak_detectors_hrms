                    ...
                    children: [
                      Flexible(
                        child: Container(
                          ...
                          // LayoutBuilder reads the chip's actual allocated
                          // width so we can drop the leading check-circle
                          // icon when the slot is too narrow to hold both
                          // icon (16 px including its right-padding) and
                          // at least a few characters of the status label.
                          // Without this, on cramped screens the outer
                          // Flexible was giving the chip ~9 px → the 16 px
                          // icon overflowed by ~7 px regardless of how
                          // strictly the Text was told to ellipsize.
                          child: LayoutBuilder(
                            builder: (context, constraints) {
                              const iconSlot = 16.0; // 12 px icon + 4 px gap
                              const minTextSlot = 18.0;
                              final showIcon = isApproved &&
                                  constraints.maxWidth >=
                                      iconSlot + minTextSlot;
                              return Row(
                                mainAxisSize: MainAxisSize.min,
                                children: [
                                  if (showIcon)
