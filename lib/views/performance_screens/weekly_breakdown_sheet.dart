...
class WeeklyBreakdownSheet extends StatefulWidget {
  ...
  @override
  State<WeeklyBreakdownSheet> createState() => _WeeklyBreakdownSheetState();
}
...
class _WeeklyBreakdownSheetState extends State<WeeklyBreakdownSheet> {
  ...
  int get _nextWeekNumber {
    ...
    return scheduled.map((t) => t.weekNumber).reduce((a, b) => a > b ? a : b) +
        1;
  }
  ...
  String _empName(String employeeId) {
    try {
      final employees = widget.vm.employees as List<Map<String, dynamic>>;
  ...
  void _addWeekTask() {
    ...
    final dueDate = widget.task.startDate.add(Duration(days: nextWeek * 7));
  ...
  Widget build(BuildContext context) {
    ...
    final completed = allTasks
        ...
        .length;
    final missed = allTasks.where((t) => t.status == TaskStatus.missed).length;
                      ...
                      children: [
                        ...
                        if (scheduled.isNotEmpty) ...[
                          ...
                          if (!widget.isEmployeeView)
                            ..._groupByEmployee(scheduled).entries.map((
                              empEntry,
                            ) {
                              ...
                              return Column(
                                ...
                                children: [
                                  ...
                                  ..._groupByMonth(empTasks).entries.map(
                                    (monthEntry) => Padding(
                          ...
                          // Employee → flat Month grouping
                            ...
                            ..._groupByMonth(scheduled).entries.map(
                              (entry) => _MonthSection(
                                month: entry.key,
                                tasks: entry.value,
                                vm: widget.vm,
              ...
              children: [
                ...
                if (isPending) ...[
                  ...
                  TextField(
                    ...
                    onChanged: (v) {
                      final idx = (vm.weeklyTasksTemp as List<WeeklyTaskModel>)
                          .indexOf(task);
                      if (idx != -1) {
                        vm.updateTempTask(idx, task.copyWith(title: v));
                      }
                    },
                  ),
                  ...
                  TextField(
                    ...
                    onChanged: (v) {
                      final idx = (vm.weeklyTasksTemp as List<WeeklyTaskModel>)
                          .indexOf(task);
                      if (idx != -1) {
                        vm.updateTempTask(idx, task.copyWith(description: v));
                      }
                    },
                  ),
                ] else ...[
                ...
                Row(
                  children: [
                    ...
                    if (task.completedAt != null) ...[
                      ...
                      const SizedBox(width: 3),
                      Text(
                        'Done ${fmt.format(task.completedAt!)}',
                        style: const TextStyle(
                          fontSize: 10,
                          color: Color(0xFF059669),
                        ),
          ...
          ElevatedButton(
            onPressed: () async {
              ...
              if (context.mounted) {
                ScaffoldMessenger.of(context).showSnackBar(
                  SnackBar(
                    content: Text('Week ${task.weekNumber} marked missed'),
